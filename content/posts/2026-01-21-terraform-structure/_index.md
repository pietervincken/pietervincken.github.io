...
In essence, using automation to improve the throughput a single team can deliver is nothing new. 
Our datacenter colleagues have been using automation since the early 90s to prevent having to go to the racks to install new software or to repeat that same installation repeatedly over a fleet of 100s of servers in the datacenter. 
...
However, probably the most asked question in the community and in enterprises is: how do we structure our IaC codebase?
...
If your organization is similar in size or approach, this should be a great jumping off point. 
...
If you don't fit these requirements, don't worry, you're likely to find good guidelines in this blogpost. 

## Definitions

This framework relies on using Git repositories to split the different component into separate manageable blocks. 
...
It's important to note that it's versioned with the parent context and should only be used to improve the codebase structure within a repository. 
...
A module should provide an organizational interpretation of the capability in an opinionated way. 
...
This prevents a hard to maintain setup with different tags and versions in a single repository and guarantees version consistency when using the module. 
...
An added benefit of a proxy or pull-through registry is that modules with unwanted licenses can be blocked before they are used in your organization's codebase. 

A simple example of this is a virtual machine module and the monitoring of that virtual machine. 
By using the composition pattern to provide these two capabilities the modules can be used separately, and their functionality isn't combined into one large module which reduces the complexity of maintaining the module. 
...
The idea is to limit the number of inputs and outputs to only the ones that a user of the module needs.
A discussed earlier, the focus of a module should be to provide a compliant way to provision infrastructure in accordance with the organizational requirements. 
...
Output variables should support linked components together in an orchestration repository.
Again, limiting the amount of output variables makes maintaining modules over time a lot easier.
...
#### An orchestration repository must always be deployable from the default branch
...
Generally, this is a bad practice as this makes the Terraform run less portable and more dependent on the system performing the Terraform run. 
It also typically creates unnecessary hassle with having configuration managed in different systems. 
...
If these are needed and a module doesn't exist for its use case (or the use case is specific to the environment) embedded modules can be used to house that complexity. 
...
As a rule of thumb, if a single plan takes more than 10 minutes, investigate splitting the repository into multiple orchestration repositories. 
...
Both create direct dependencies on other infrastructure deployments to be successful.
...
When using data blocks, the rollout will check for the existence and access to the resource. 
While this prevents rollouts from occurring without the infrastructure being present, it does require that the automation has access to the actual underlying infrastructure to fetch the data through the data block. 

Finally, there is the [remote state data block](https://developer.hashicorp.com/terraform/language/state/remote-state-data).
While this seemingly allows to have the referential integrity without needing access to the actual underlying infrastructure, it introduces a direct dependency between both automations.
...
- Prevent the usage of remote state data block at all costs. They introduce hard dependencies between runs which are hard to debug and often cross team boundaries which makes coordinating rollouts even harder. 
...
Depending on your industry, usage pattern and organization policy w.r.t. permitted licenses. 

Next to checking the licenses of public modules, it's also important to add a license to the internally build modules. 
...
This framework with its rules is a suggestion on how to structure your Terraform setups.
It's by no means a one size fits all and it doesn't accommodate all setups. 
...
- Offer an IaC as a service to your workload teams. Allow them to provision their own infrastructure.
